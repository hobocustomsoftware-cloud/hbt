...
class PrintDocumentListCreateView(
...
class PrinterProfileListCreateView(
    OrganizationSchedulingMixin, generics.ListCreateAPIView
):
...
class PrintTemplateListCreateView(
    OrganizationSchedulingMixin, generics.ListCreateAPIView
):
...
class PrintAcknowledgeView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        request=PrintAttemptSerializer,
        responses=PrintAttemptSerializer,
        operation_id="print_document_acknowledge",
    )
    def post(self, request, organization_id, document_id, version=None):
        organization, _ = self.organization_and_membership()
        document = get_object_or_404(
            PrintDocument, pk=document_id, organization=organization
        )
        serializer = PrintAttemptSerializer(
            data=request.data,
...
class TripCloseView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        request=TripCloseRequestSerializer,
        responses=TripClosingSerializer,
        operation_id="trip_close",
    )
    def post(self, request, organization_id, trip_id, version=None):
        ...
        serializer = TripCloseRequestSerializer(
            data=request.data, context={"trip": trip, "request": request}
        )
        serializer.is_valid(raise_exception=True)
...
class SettlementCreateView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        ...
        responses=CashSettlementSerializer,
        operation_id="cash_settlement_create",
    )
    def post(self, request, organization_id, trip_id, version=None):
        organization, _ = self.organization_and_membership()
        trip = get_object_or_404(Trip, pk=trip_id, organization=organization)
...
class SettlementActionView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        request=SettlementActionSerializer,
        responses=CashSettlementSerializer,
        operation_id="cash_settlement_action",
    )
    def post(self, request, organization_id, settlement_id, version=None):
        organization, _ = self.organization_and_membership()
        settlement = get_object_or_404(
            CashSettlement, pk=settlement_id, organization=organization
        )
        serializer = SettlementActionSerializer(
            data=request.data,
            context={"settlement": settlement, "request": request},
        )
        serializer.is_valid(raise_exception=True)
        return Response(CashSettlementSerializer(serializer.save()).data)
...
class ReportExportView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter("report", str, required=True),
        ...
        operation_id="organization_report_export_csv",
    )
    def get(self, request, organization_id, version=None):
        organization, _ = self.organization_and_membership()
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="hbt-{report}-{start}-{end}.csv"'
        )
    ...
    def _rows(organization, report, start, end):
        if report == "trips":
            ...
            ).order_by("service_date", "planned_departure_at"):
                yield (
                    trip.trip_number,
                    trip.service_date,
                    trip.status,
        ...
        if report == "payments":
            yield (
                "payment_number", "confirmed_at", "method", "amount",
                "currency", "booking_id", "cargo_shipment_id",
            )
            for payment in PaymentRecord.objects.filter(
                ...
                yield (
                    ...
                    payment.currency,
                    payment.booking_id,
                    payment.cargo_shipment_id,
                )
            return
        if report == "cargo":
            yield (
                "shipment_number", "created_at", "status", "pieces",
                "weight_kg", "total_charge", "currency",
            )
            for shipment in CargoShipment.objects.filter(
                organization=organization,
        ...
        trips = Trip.objects.filter(
            organization=organization, service_date__range=(start, end)
        )
        tickets = Ticket.objects.filter(
            organization=organization,
            trip__service_date__range=(start, end),
        ).exclude(status=Ticket.Status.CANCELLED)
        cargo = CargoShipment.objects.filter(
            organization=organization,
            created_at__date__range=(start, end),
        ...
        yield ("metric", "value")
        yield ("date_from", start)
        yield ("date_to", end)
        yield ("trip_count", trips.count())
        yield ("closed_trip_count", trips.filter(status=Trip.Status.CLOSED).count())
        yield ("ticket_count", tickets.count())
        yield ("cargo_count", cargo.count())
        yield ("confirmed_payment_total", payments.aggregate(total=Sum("amount"))["total"] or 0)
...
class SyncBootstrapView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        responses=SyncBootstrapSerializer,
        operation_id="offline_sync_bootstrap",
    )
    def get(self, request, organization_id, version=None):
        organization, _ = self.organization_and_membership()
        ...
        if since:
            cargo = cargo.filter(updated_at__gt=since)
            trips = trips.filter(updated_at__gt=since)
        return Response({
            "server_time": timezone.now(),
            "cargo_shipments": list(
                cargo.values("id", "shipment_number", "tracking_code", "status",
                             "assigned_trip_id", "updated_at")[:1000]
            ),
            "trips": list(
                trips.values("id", "trip_number", "status", "service_date",
                             "updated_at")[:500]
            ),
        })
...
def _backup_age_seconds():
    ...
    modified_at = timezone.datetime.fromtimestamp(
        path.stat().st_mtime, tz=dt_timezone.utc
    )
...
class MonitoringDashboardView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        responses=MonitoringDashboardSerializer,
        operation_id="organization_monitoring_dashboard",
    )
    def get(self, request, organization_id, version=None):
        organization, _ = self.organization_and_membership()
        now = timezone.now()
        ...
        push_backlog = Notification.objects.filter(
            organization=organization,
        ...
        sync_backlog = SyncOperation.objects.filter(
            organization=organization, status=SyncOperation.Status.RECEIVED
        )
        webhook_backlog = PaymentWebhookEvent.objects.filter(
        ...
        return Response({
            "service_overview": {
                "generated_at": now,
                "active_devices_24h": Device.objects.filter(
                    user__organization_memberships__organization=organization,
                    user__organization_memberships__status="active",
                    status=Device.Status.ACTIVE,
                    last_seen_at__gte=last_24h,
                ).distinct().count(),
                "pending_push_notifications": push_backlog.count(),
                "push_queue_oldest_seconds": _oldest_age_seconds(
                    push_backlog, "available_at", now
                ),
                "pending_work_items": PendingWorkItem.objects.filter(
                    organization=organization,
                    status=PendingWorkItem.Status.PENDING,
                ).count(),
                "sync_backlog_count": sync_backlog.count(),
                "sync_backlog_oldest_seconds": _oldest_age_seconds(
                    sync_backlog, "created_at", now
                ),
                "sync_conflicts_24h": SyncOperation.objects.filter(
                    organization=organization,
                    status__in=[
                ...
                "payment_reconciliation_backlog": PaymentIntent.objects.filter(
                    organization=organization,
                    status=PaymentIntent.Status.REQUIRES_RECONCILIATION,
                ).count(),
                "webhook_backlog_count": webhook_backlog.count(),
                "webhook_backlog_oldest_seconds": _oldest_age_seconds(
                    webhook_backlog, "created_at", now
                ),
            },
            "business_reliability": {
                ...
                "expired_bookings_today": Booking.objects.filter(
                    organization=organization,
                ...
                "rejected_boardings_24h": BoardingRecord.objects.filter(
                    organization=organization,
                    status=BoardingRecord.Status.REJECTED,
                    created_at__gte=last_24h,
                ).count(),
                "cargo_exceptions_24h": CargoShipment.objects.filter(
                    organization=organization,
                ...
                "refunds_in_progress": RefundRequest.objects.filter(
                    organization=organization,
                    status__in=[
                ...
                "subscription_past_due": SubscriptionInvoice.objects.filter(
            ...
            "infrastructure": {
                ...
                "backup_age_seconds": _backup_age_seconds(),
                "certificate_expires_in_days": _certificate_expiry_days(),
            },
        })
...
class MonitoringAlertsView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        responses=MonitoringAlertListSerializer,
        operation_id="organization_monitoring_alerts",
    )
    def get(self, request, organization_id, version=None):
        organization, _ = self.organization_and_membership()
        now = timezone.now()
        ...
        certificate_days = _certificate_expiry_days()
        alerts = [
            {
                "code": "api_readiness",
                "severity": "critical",
                "status": "ok" if database_status == "ok" else "triggered",
                "threshold": "database must be reachable",
                "value": database_status,
                "message": (
                    "Database-backed readiness is healthy."
                    if database_status == "ok"
                    else "Database-backed readiness failed."
                ),
            },
            {
                "code": "payment_webhook_backlog",
                "severity": "critical",
                "status": (
                    "triggered"
                    if _oldest_age_seconds(webhook_backlog, "created_at", now) > 300
                    else "ok"
                ),
                "threshold": "oldest received webhook <= 300 seconds",
                "value": str(_oldest_age_seconds(webhook_backlog, "created_at", now)),
                "message": "Payment webhook backlog age within threshold.",
            },
            {
                "code": "offline_sync_backlog",
                "severity": "warning",
                "status": (
                    "triggered"
                    if _oldest_age_seconds(sync_backlog, "created_at", now) > 1800
                    else "ok"
                ),
                "threshold": "oldest received offline sync <= 1800 seconds",
                "value": str(_oldest_age_seconds(sync_backlog, "created_at", now)),
                "message": "Offline sync backlog age within threshold.",
            },
            {
                "code": "backup_age",
                "severity": "critical",
                "status": (
                    "unavailable" if backup_age is None
                    else "triggered" if backup_age > 86400 else "ok"
                ),
                "threshold": "latest backup age <= 86400 seconds",
                "value": "unknown" if backup_age is None else str(backup_age),
                "message": (
                    "Backup age evidence is unavailable."
                    if backup_age is None
                    else "Latest backup age is within threshold."
                ),
            },
            {
                ...
                "severity": "warning",
                "status": (
                    "unavailable" if certificate_days is None
                    else "triggered" if certificate_days < 14 else "ok"
                ),
                "threshold": "certificate expiry >= 14 days",
                "value": (
                    "unknown" if certificate_days is None else str(certificate_days)
                ),
                "message": (
                    "Certificate expiry evidence is unavailable."
                    if certificate_days is None
                    else "Certificate expiry is within threshold."
                ),
            },
            {
                "code": "push_queue_backlog",
                "severity": "warning",
                "status": (
                    "triggered"
                    if _oldest_age_seconds(push_backlog, "available_at", now) > 300
                    else "ok"
                ),
                "threshold": "oldest push queue item <= 300 seconds",
                "value": str(_oldest_age_seconds(push_backlog, "available_at", now)),
                "message": "Push queue age is within threshold.",
            },
        ]
        for alert in alerts:
            if alert["status"] == "triggered":
                if alert["code"] == "payment_webhook_backlog":
                    alert["message"] = "Payment webhook backlog exceeded 5 minutes."
                elif alert["code"] == "offline_sync_backlog":
                    alert["message"] = "Offline sync backlog exceeded 30 minutes."
                elif alert["code"] == "backup_age":
                    alert["message"] = "Latest backup evidence is older than 24 hours."
                elif alert["code"] == "certificate_expiry":
                    alert["message"] = "TLS certificate expires in fewer than 14 days."
                elif alert["code"] == "push_queue_backlog":
                    alert["message"] = "Push queue backlog exceeded 5 minutes."
