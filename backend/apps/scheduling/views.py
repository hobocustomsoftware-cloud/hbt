...
from .serializers import (
    ConductorAssignmentSerializer,
    DriverAssignmentSerializer,
    GenerateTripSerializer,
    ScheduleSerializer,
    TripAssignmentEventSerializer,
    TripOperationSerializer,
    TripOperationalEventSerializer,
    TripOperationResponseSerializer,
    TripSerializer,
    StopReachedSerializer,
...
class ScheduleListCreateView(
    ...
    def get_queryset(self):
        organization, _ = self.organization_and_membership()
        return Schedule.objects.filter(organization=organization).select_related(
            "route"
        )
...
class ScheduleDetailView(
    ...
    def perform_update(self, serializer):
        ...
        self.audit(
            "schedule.updated", schedule, before={"status": old_status}
        )
...
class TripListCreateView(OrganizationSchedulingMixin, generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        organization, _ = self.organization_and_membership()
        return Trip.objects.filter(organization=organization).select_related(
...
class GenerateTripView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        request=GenerateTripSerializer,
        responses={201: TripSerializer},
        operation_id="trip_generate_from_schedule",
    )
    def post(self, request, organization_id, schedule_id, version=None):
        organization, _ = self.organization_and_membership()
        schedule = get_object_or_404(
            Schedule, pk=schedule_id, organization=organization
        )
        ...
        return Response(
            TripSerializer(trip, context={"organization": organization}).data,
...
class TripAssignmentHistoryView(
    ...
    def get_queryset(self):
        ...
        trip = get_object_or_404(
            Trip, pk=self.kwargs["trip_id"], organization=organization
        )
...
def require_trip_scope(membership, trip, permission):
    ...
    allowed = Q(scope_type=MembershipRole.ScopeType.COMPANY) | Q(
        scope_type=MembershipRole.ScopeType.ASSIGNED_TRIP,
        scope_id=trip.id,
    )
    if trip.vehicle_id:
        allowed |= Q(
            scope_type=MembershipRole.ScopeType.BRANCH,
            scope_id=trip.vehicle.branch_id,
        )
    if (
        (trip.driver_id and trip.driver.staff.membership_id == membership.id)
        or (
            trip.conductor_id
            and trip.conductor.staff.membership_id == membership.id
        )
    ):
...
class TripOperationView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        request=TripOperationSerializer,
        responses=TripOperationResponseSerializer,
    )
    def post(self, request, organization_id, trip_id, version=None):
        ...
        serializer = TripOperationSerializer(
            data=request.data,
            context={
                "trip": trip,
        ...
        return Response(
            {
                "trip": TripSerializer(
                    trip, context={"organization": organization}
                ).data,
                "event": TripOperationalEventSerializer(serializer.event).data,
            }
        )
...
class StopReachedView(OrganizationSchedulingMixin, APIView):
    ...
    @extend_schema(
        request=StopReachedSerializer,
        responses=TripOperationResponseSerializer,
        operation_id="trip_stop_reached",
    )
    def post(self, request, organization_id, trip_id, stop_id, version=None):
        ...
        route_stop = get_object_or_404(
            RouteStop, pk=stop_id, route=trip.route
        )
        serializer = StopReachedSerializer(
            data=request.data,
            context={
                "trip": trip,
...
class TripOperationalEventListView(
    OrganizationSchedulingMixin, generics.ListAPIView
):
    ...
    def get_queryset(self):
        ...
        return trip.operational_events.select_related(
            "route_stop", "recorded_by"
        )
