...
| M5 Quality & cleanup | 🔄 M5a DONE; M5b-001/2/3 DONE; M5c-001 DONE (F-24 part); F-23b + rest pending | F-19 indexes+pagination (M5b-001/2). F-23 dead-code removal (M5b-003). F-24(part): 11 new controller tests — AuthController (restore/signIn/signOut incl. expired-token + no-token paths), OrgController (context load, saved-org pick, fallback), SessionController facade (signIn composition, signOut clears all). Business suite 82/82. Remaining M5: cargo/trip/routes screen tests (F-24), localization (F-25), search (F-26) |
