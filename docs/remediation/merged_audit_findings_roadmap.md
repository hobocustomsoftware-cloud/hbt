...
| M5 Quality & cleanup | 🔄 M5a DONE (F-18 hook + F-09 idle timeout); rest pending | F-18: env-gated CrashReporter wired into both apps (no-op w/o DSN; F-18b vendor SDK one-file change). F-09: 15-min idle lock overlay + re-auth guard (cert pinning F-09b deferred — needs build infra). Remaining M5: dead widgets (F-23), tests (F-24), localization (F-25), search (F-26), composite indexes (F-19), crash SDK (F-18b) |
