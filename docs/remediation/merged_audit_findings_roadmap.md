...
| M5 Quality & cleanup | 🔄 M5a DONE; M5b-001 DONE; M5b-002 DONE; M5b-003 DONE (F-23 dead code); F-23b rename deferred; rest pending | F-19 indexes + pagination (M5b-001/2). F-23(part): removed 6 dead widget files (data_table, search_field, permission_guard, printer_dialog, section_header, widgets.dart barrel) + unused routing/routes.dart — all verified zero consumers (analyze + 71 tests green). F-23b `features/business` → `features/shell` rename deferred (style-only churn, breaking import paths — rules 1/8). Remaining M5: tests (F-24), localization (F-25), search (F-26); duplicate error/loading widgets (async_views vs error_states/loading) documented as duplication debt |
