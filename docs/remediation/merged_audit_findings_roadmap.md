...
| M5 Quality & cleanup | 🔄 M5a DONE; M5b-001 DONE (F-19 indexes); M5b-002 DONE (F-19 pagination); rest pending | F-18 hook + F-09 idle timeout (M5a). F-19(part 1): composite indexes Ticket+PaymentRecord. F-19(part 2): `ApiClient.getAllPages()` follows DRF `next` links; wired into cargo worklist, route list, refund service (lists no longer truncate at page 1). Remaining M5: dead widgets (F-23), tests (F-24), localization (F-25), search (F-26) |
