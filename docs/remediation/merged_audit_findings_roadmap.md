...
| M5 Quality & cleanup | 🔄 M5a DONE (F-18 hook + F-09 idle timeout); M5b-001 DONE (F-19 composite indexes); rest pending | F-18: env-gated CrashReporter wired into both apps (no-op w/o DSN; F-18b vendor SDK one-file change). F-09: 15-min idle lock overlay + re-auth guard (cert pinning F-09b deferred — needs build infra). F-19(part): composite indexes added to Ticket (org+status+issued_at) and PaymentRecord (org+status+created_at) — verified already present on Booking/Trip/CargoShipment/Route/Schedule/Vehicle/Branch/Feedback/SyncOperation. Remaining M5: cargo/route/refund list pagination (F-19 part 2), dead widgets (F-23), tests (F-24), localization (F-25), search (F-26) |
